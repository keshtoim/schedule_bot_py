...
| `OWNER_CHAT_ID` | нет | твой chat_id — багрепорты + доступ к `/announce` и `/maintenance`. Узнать: `/id` боту |
| `REMINDER_DEFAULT` | нет | время напоминания «пары на завтра» по умолчанию (`ЧЧ:ММ` или `off`), 20:00 |
...
| `reminders.json` | во сколько кому слать «пары на завтра» |
| `reminder-sent.json` | кому сегодня уже отправили напоминание (сбрасывается за ночь) |
| `maintenance` | существует = режим техработ включён (`/maintenance`) |
...
## Техработы и рассылки (только владелец)

Команды доступны только с `OWNER_CHAT_ID`.

- **`/announce <текст>`** — разослать сообщение всем пользователям (с подтверждением).
- **`/maintenance`** — вкл/выкл режим техработ: бот **остаётся живым**, но всем,
  кроме тебя, отвечает «вернусь позже». Ты продолжаешь пользоваться как обычно.

Порядок для обновления с даунтаймом:
```
/announce Сегодня в 22:00 бот ненадолго уйдёт на обновление, вернусь через 15 минут
/maintenance                      # включить — юзеры видят «техработы»
# ... git pull, docker compose up -d --build ...
/maintenance                      # выключить
```

Бот **не выключает себя сам** — да это и не сработало бы: `restart: unless-stopped`
тут же поднимет контейнер обратно. Если нужно реально остановить процесс
(миграция, что-то серьёзное) — `docker compose stop bot`, потом `start bot`.
Режим техработ переживает перезапуск (флаг в волюме).
